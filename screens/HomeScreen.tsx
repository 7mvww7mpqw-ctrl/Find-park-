...
import { Search, Navigation, Clock, Car, Zap, MapPin, Bell, Sparkles, Trophy, Eye, Mic, Volume2, X, ArrowLeft, Calendar, Timer, CreditCard, Activity, ShieldCheck, Info, BarChart3, Radio, Umbrella, Crown, Wallet, Cpu, CloudSun, Signal, AlertCircle, Send, MessageSquareText, Heart } from 'lucide-react';
...
export const HomeScreen: React.FC<HomeScreenProps> = ({ setScreen }) => {
    ...
    const handleAIVoiceRequest = async (e?: React.FormEvent) => {
        ...
        // الرد النصي المطلوب: "سيتم توجيهك لأقرب موقف للمول"
        let targetText = "سيتم توجيهك لأقرب موقف للمول";
        let targetId = "1"; // الموقف الافتراضي للمول

        // منطق التوجيه بناءً على الكلمات المفتاحية
    ...
    return (
        <div className="relative h-full w-full flex flex-col bg-white overflow-hidden">
            ...
            <div className="absolute top-0 left-0 right-0 z-30 pt-12 glass-light border-b border-slate-100 shadow-sm">
                <div className="px-6 pb-4 flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                        {congestionAlert && (
                            <div className="bg-rose-100 text-rose-600 px-3 py-1.5 rounded-xl flex items-center gap-2 border border-rose-200">
                                <AlertCircle size={14} className="animate-pulse" />
                                <span className="text-[10px] font-black uppercase tracking-widest">زحام</span>
                            </div>
                        )}
                        <button onClick={() => setScreen(ScreenState.SETTINGS)} className="p-2.5 rounded-xl border bg-slate-50 border-slate-100 text-slate-400">
                            <Bell size={18} />
                        </button>
                    </div>
                </div>
                ...
                <div className="px-6 pb-4 flex gap-3 overflow-x-auto no-scrollbar">
                    {CATEGORIES.map(cat => (
                        <button key={cat.id} onClick={() => setFilterCategory(cat.id)} className={`flex-shrink-0 px-4 py-2 rounded-xl text-xs font-bold transition-all border ${filterCategory === cat.id ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg shadow-indigo-100' : 'bg-white border-slate-100 text-slate-400'}`}>{cat.label}</button>
                    ))}
                </div>
            </div>
            ...
            <div className="flex-1 relative bg-slate-50 map-grid overflow-hidden">
                <div className="absolute top-[15%] right-[5%] w-72 h-72 bg-rose-500/10 rounded-full blur-[90px] pointer-events-none animate-pulse"></div>
                <div className="absolute bottom-[30%] left-[20%] w-60 h-60 bg-emerald-500/10 rounded-full blur-[80px] pointer-events-none"></div>

                <div className="absolute top-48 left-6 z-20 w-32 animate-fade-in pointer-events-none opacity-40">
                    <div className="bg-slate-900/90 backdrop-blur-md p-3 rounded-2xl border border-white/10 shadow-2xl">
                        {sensorLogs.map((log, i) => <p key={i} className="text-[7px] font-mono text-emerald-400 overflow-hidden">{`> ${log}`}</p>)}
                    </div>
                </div>

                {filteredSpots.map(spot => (
                    <button key={spot.id} onClick={() => setSelectedSpot(spot)} style={{ left: `${spot.x}%`, top: `${spot.y}%` }} className={`absolute transform -translate-x-1/2 -translate-y-1/2 transition-all duration-700 ${selectedSpot?.id === spot.id ? 'scale-125 z-20' : 'z-10'}`}>
                        ...
                        <div className={`group relative p-3 rounded-2xl border-4 shadow-2xl flex flex-col items-center transition-all ${spot.status === ParkingStatus.AVAILABLE ? 'bg-white border-indigo-600 text-indigo-600 shadow-indigo-100' : 'bg-slate-100 border-slate-300 text-slate-400 opacity-60'}`}>
                            {spot.isFavorite && (
                                <div className="absolute -top-2 -right-2 bg-rose-500 text-white p-1 rounded-full shadow-lg border border-white">
                ...
                {aiThinking && (
                    <div className="absolute inset-0 z-40 bg-white/95 backdrop-blur-2xl flex flex-col items-center justify-center p-10 animate-fade-in text-center">
                        <div className="w-24 h-24 bg-indigo-50 rounded-full flex items-center justify-center border border-indigo-100 animate-pulse-gentle mb-8">
                        ...
                        <div className="bg-indigo-600 p-10 rounded-[3.5rem] shadow-2xl shadow-indigo-200 border border-indigo-500 relative transform transition-transform hover:scale-105">
                            <p className="text-white font-black text-xl leading-relaxed italic">{assistantText}</p>
                ...
                {isAsking && (
                    <div className="absolute inset-0 z-50 bg-slate-900/60 backdrop-blur-md flex items-end p-6 animate-fade-in">
                        <div className="w-full bg-white rounded-[3.5rem] p-8 shadow-2xl animate-slide-up mb-24 border border-slate-100">
                            ...
                            <p className="text-center text-slate-400 text-[10px] font-black mt-8 tracking-[0.3em] uppercase italic">FindPark AI Assistant</p>
                        </div>
                    </div>
                )}
                ...
                {!selectedSpot && !aiThinking && !isAsking && (
                    <div className="absolute bottom-32 left-0 right-0 flex flex-col items-center gap-3 z-20">
                        <button onClick={() => setIsAsking(true)} className="bg-indigo-600 text-white px-12 py-6 rounded-full font-black text-xl flex items-center gap-4 shadow-[0_30px_60px_rgba(79,70,229,0.4)] hover:scale-105 active:scale-95 transition-all border-b-4 border-indigo-800">
                            <Sparkles size={28} /> اسألي المساعد الذكي ✨
                        </button>
                    </div>
                )}
            </div>
            ...
            {/* Selection Drawer */}
            {selectedSpot && !aiThinking && !isAsking && (
                <div className="absolute inset-x-0 bottom-0 bg-white rounded-t-[4rem] p-10 pb-36 animate-slide-up z-30 shadow-[0_-30px_100px_rgba(0,0,0,0.1)] border-t border-slate-100">
                    ...
                    <div className="flex justify-between items-start mb-10">
                        <div>
                            <div className="flex items-center gap-3 mb-3">
                                ...
                                <span className="bg-indigo-50 text-indigo-600 px-3 py-1 rounded-lg text-xs font-black uppercase">{selectedSpot.type}</span>
                    ...
                    <button onClick={() => setScreen(ScreenState.BOOKING)} className="w-full bg-slate-900 text-white py-7 rounded-[2.5rem] font-black text-2xl flex items-center justify-center gap-5 shadow-2xl hover:bg-black active:scale-[0.98] transition-all group">
                        <Car size={32} className="group-hover:translate-x-2 transition-transform" /> تأكيد الحجز المسبق ({selectedSpot.price})
                    </button>
                </div>
            )}
        </div>
