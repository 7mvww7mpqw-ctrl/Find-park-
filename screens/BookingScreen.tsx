        ...
        return (
            <div className="h-full w-full bg-slate-50 p-8 flex flex-col items-center justify-center text-center animate-fade-in overflow-hidden">
                ...
                <div className="space-y-4 w-full">
                    <button onClick={() => setScreen(ScreenState.DIGITAL_KEY)} className="w-full bg-indigo-600 text-white py-5 rounded-2xl font-black text-lg shadow-[0_20px_40px_rgba(79,70,229,0.3)] flex items-center justify-center gap-3 active:scale-95 transition-all">
                        <QrCode size={24} /> عرض التذكرة والباركود
                    </button>
    ...
    return (
        <div className="h-full w-full bg-white flex flex-col">
            ...
            <div className="p-6 flex-1 overflow-y-auto">
                <div className="mb-10">
                    ...
                    <div className="flex gap-4 overflow-x-auto pb-4 no-scrollbar">
                        {[1, 2, 4, 8, 24].map(hr => (
                            <button key={hr} onClick={() => setDuration(hr)} className={`flex-shrink-0 w-24 h-28 rounded-3xl border-2 flex flex-col items-center justify-center transition-all ${duration === hr ? 'border-indigo-600 bg-indigo-50 text-indigo-600 shadow-xl scale-105' : 'border-gray-50 bg-white text-gray-400'}`}>
            ...
            <div className="p-8 bg-white border-t border-gray-50">
                <button onClick={() => setConfirmed(true)} className="w-full bg-indigo-600 text-white py-5 rounded-[1.8rem] font-black text-lg shadow-[0_20px_40px_rgba(79,70,229,0.3)] hover:scale-[1.02] active:scale-95 transition-all">تأكيد الحجز والدفع</button>
            </div>
        </div>
    );
};
